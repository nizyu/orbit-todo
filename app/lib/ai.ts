...
export async function suggestTaskRefinement(
	...
	const prompt = `
[依頼内容]
1. 以下のタスクに関する情報と背景を元に、タスクを達成するために必要な具体的なチェックリスト（サブタスク）と、このタスクの完了条件（Done Criteria）を生成してください。
    - このタスクが「どうなれば完了と言えるか」を明確に言語化し \`doneCriteria\` に設定してください。
    - チェックリストの項目数は5〜7個程度に抑えるようにしてください。
	- チェックリストの各項目は以下の条件を満たすようにしてください。
		- タスクを達成するための意味のあるマイルストーンや成果物単位に分割すること
		- 担当者が「今日はこれを終わらせた」と実感できるキリの良い粒度（1〜2時間程度）とすること
		- 担当者が能動的に処理をできること（何かを待つなどのタスクは不要です）
		- 具体的でわかりやすいこと
...
[期待する出力フォーマット]
返却値は必ずJSON形式のオブジェクトとしてください。オブジェクトは \`checklist\` (CheckListItemの配列), \`doneCriteria\` (文字列), \`advice\` (文字列) を持つオブジェクトとしてください。
...
[出力例]
{
  ...
  "doneCriteria": "全ての要件がドキュメントにまとまり、実装担当者が作業を開始できる状態になること",
  "advice": "タスクの粒度が適切か"
}
		...
		if (result.success) {
			...
			return {
				checklist: Array.isArray(parsed?.checklist)
					...
					: [],
				doneCriteria: typeof parsed?.doneCriteria === "string" ? parsed.doneCriteria : undefined,
				advice:
...
export async function suggestTaskFromInbox(
	...
	// カテゴリの一覧をプロンプト用に整形する
	const categoriesListText = categories.length > 0
		? categories
			.map((cat) => {
				const parent = cat.parentId ? categories.find((c) => c.id === cat.parentId) : null;
				const parentName = parent ? ` (親カテゴリ: ${parent.name})` : "";
				const desc = cat.description ? `, 説明: ${cat.description}` : "";
				return `- ID: "${cat.id}", 名前: "${cat.name}"${parentName}${desc}`;
			})
			.join("\n")
		: "なし";
...
- このタスクが「どうなれば完了と言えるか」を明確に言語化し \`doneCriteria\` に設定する。
- タスクを達成するために必要な具体的なチェックリスト（サブタスク）を生成する（5〜7個程度）。
- 各チェックリストは担当者が能動的に処理でき、具体的でわかりやすく、1つあたり1〜2時間程度で終わるマイルストーン単位の粒度にすること。順序も考慮すること。
- [利用可能なカテゴリ一覧] を参考に、このタスクに最も適したカテゴリを1つ選び、そのカテゴリの ID を \`categoryId\` フィールドに設定してください。適切なカテゴリがない、または判断が難しい場合は \`null\` にしてください。

[利用可能なカテゴリ一覧]
${categoriesListText}

[期待する出力フォーマット]
返却値は必ずJSON形式のオブジェクトとしてください。オブジェクトは \`title\` (文字列), \`description\` (文字列), \`doneCriteria\` (文字列), \`checklist\` (CheckListItemの配列), \`categoryId\` (文字列またはnull) を持つオブジェクトとしてください。
Markdownのコードブロック (\`\`\`json ... \`\`\`) は使用せず、純粋なJSON配列文字列のみを出力してください。無理な場合はコードブロックで囲っても構いません。
...
[出力例]
{
  ...
  "doneCriteria": "〇〇機能が本番環境にデプロイされ、ユーザーが正常に利用できる状態になること",
  "checklist": [
    { "title": "要件を整理する", "completed": false },
    { "title": "関連ドキュメントを読む", "completed": false }
  ],
  "categoryId": "特定のカテゴリID"
}
	...
	try {
		...
		if (result.success) {
			...
			return {
				...
				doneCriteria: typeof parsed?.doneCriteria === "string" ? parsed.doneCriteria : "",
				checklist: Array.isArray(parsed?.checklist)
					? parsed.checklist.map((item: any) => ({
							title: typeof item?.title === "string" ? item.title : "No Title",
							completed: false,
